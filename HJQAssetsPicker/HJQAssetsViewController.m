...
- (void)setModel:(HImageModel *)model
{
    _model = model;
    ...
    }else
    {
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            [[HJQImageManager manager] getPhotoWithAsset:model.assets photoWidth:self.frame.size.width isSynchronous:NO completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
@interface HJQAssetsViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,YGrideSelectedDelegate,PHPhotoLibraryChangeObserver>
{
...
@implementation HJQAssetsViewController
...
- (void)customAssets
{
    ...
    __block typeof(HJQAssetsViewController*)wealSelf = self;
    
    [[HJQImageManager manager] getAssetsFromFetchResult:_assetsGroup.result completion:^(NSArray<YAssetsModel *> *models) {
        
        for (int i = 0;i<models.count;i++) {
            
            YAssetsModel *model = models[i];
            
            HImageModel *photoModel = [HImageModel new];
...
- (HImageModel *)assetAtIndexPath:(NSIndexPath *)indexPath
{
