...
@implementation HJQImageManager

+ (instancetype)manager {
    
    static HJQImageManager *manager;
...
- (void)getPostImageWithAlbumModel:(YAlbumModel *)model completion:(void (^)(UIImage *))completion {
    if (IOS9Later) {
        [[HJQImageManager manager] getPhotoWithAsset:[model.result lastObject] photoWidth:75 isSynchronous:NO completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
